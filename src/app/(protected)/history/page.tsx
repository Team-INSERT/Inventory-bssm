  ...
  return (
    <div className="space-y-6 pb-24 sm:pb-8">
      <div className="flex flex-col gap-1 sm:gap-2">
        <h1 className="text-2xl sm:text-3xl font-black tracking-tighter text-gray-900 dark:text-white">
          나의 대여 기록
        </h1>
          ...
          <tbody className="divide-y divide-black/5 dark:divide-zinc-800">
            {!txs || txs.length === 0 ? (
              <tr>
                <td
                  colSpan={5}
                  className="px-6 py-12 text-center text-gray-500 font-medium font-medium"
                >
                  대여 현황이 없습니다.
                </td>
              </tr>
                ...
                return (
                  ...
                  >
                    <td className="px-6 py-4 font-bold text-gray-900 dark:text-white">
                      {tx.items?.name}{" "}
                      <span className="text-xs text-gray-400 ml-1 font-medium italic">
                        ({tx.items?.category})
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 font-black text-lg tabular-nums">
                      {tx.quantity}
                    </td>
        ...
        {!txs || txs.length === 0 ? (
          ...
          txs.map((tx: any) => {
            ...
            return (
              <div
                key={tx.id}
                className="p-6 bg-white dark:bg-zinc-900 rounded-3xl border border-black/5 dark:border-zinc-800 shadow-sm space-y-4"
              >
                <div className="flex justify-between items-start">
                  ...
                  <span
                    className={`px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-sm ${
                      tx.type === "OUT"
                        ? "bg-orange-500/10 text-orange-600 border border-orange-500/20"
                        : tx.type === "IN"
                          ? "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20"
