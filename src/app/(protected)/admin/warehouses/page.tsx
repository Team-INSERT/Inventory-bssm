    ...
    <div className="space-y-6 sm:space-y-8">
      <div>
        ...
        <p className="text-xs sm:text-sm text-gray-500 font-medium tracking-tight">
          재고가 물리적으로 보관되는 창고 목록입니다. 새로운 창고를 추가하거나
          삭제할 수 있습니다.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-1 border border-black/5 rounded-xl sm:rounded-2xl bg-white p-4 sm:p-6 shadow-sm dark:bg-zinc-900 dark:border-zinc-800 h-fit">
          <h2 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4 flex items-center gap-2">
            <Plus className="w-4 sm:w-5 h-4 sm:h-5 text-blue-500" />새 창고 등록
          </h2>
          <form
            action={async (fd) => {
              "use server";
          ...
          >
            <div>
              ...
              <input
                name="name"
                required
                className="w-full rounded-md border border-black/5 p-2 text-xs sm:text-sm dark:bg-zinc-800 dark:border-zinc-700"
                placeholder="예: 1번 서버실"
              />
            </div>
            <button
              type="submit"
              className="w-full bg-blue-600 text-white rounded-md p-2 text-xs sm:text-sm font-medium hover:bg-blue-700 transition-colors"
            >
              등록하기
            </button>
          </form>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs sm:text-sm whitespace-nowrap">
              <thead className="bg-gray-50 text-gray-700 dark:bg-zinc-800 dark:text-gray-300 uppercase font-semibold text-[10px] sm:text-xs">
                <tr>
                  <th className="px-3 sm:px-4 py-3">창고명</th>
                  ...
                  warehouses.map((wh: any) => (
                    ...
                    >
                      <td className="px-3 sm:px-4 py-3 font-medium flex items-center gap-2">
                        <Warehouse className="w-3 sm:w-4 h-3 sm:h-4 text-gray-400" />
                        {wh.name}
                      </td>
