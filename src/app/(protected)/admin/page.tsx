...
export default async function AdminDashboard() {
  ...
  // Fetch summary data
  const { data: inventory } = await supabase.from("inventory").select("*, items(*), warehouses(*)");
  ...
  const totalItems =
    inventory?.reduce((acc: number, curr: any) => acc + curr.quantity, 0) || 0;
  const lowStockItems =
    inventory?.filter((item: any) => item.quantity <= item.items?.min_stock).length ||
    0;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-sm sm:text-base text-gray-500 font-medium mt-1">
            시스템 전체 재고 및 사용자 활동을 실시간 모니터링합니다.
          </p>
        </div>
        <div className="flex items-center gap-2 self-start px-4 py-2 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/30 rounded-2xl">
          ...
          <span className="text-[10px] sm:text-xs font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-widest text-nowrap">
            시스템 정상 가동 중
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {[
          ...
          <motion.div
            ...
            className="p-4 sm:p-6 bg-white dark:bg-zinc-900 rounded-2xl sm:rounded-3xl border border-black/5 dark:border-zinc-800 shadow-sm transition-all hover:shadow-lg hover:shadow-blue-500/5"
          >
            <div
              className={
                stat.bg +
                " w-10 h-10 sm:w-12 sm:h-12 rounded-2xl flex items-center justify-center mb-3 sm:mb-4 " +
                stat.color
              }
            >
            ...
            <p className="text-lg sm:text-2xl font-black text-gray-900 dark:text-white mt-1">
              {stat.value}
            </p>
          </motion.div>
        ))}
                ...
                >
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                    <div className="flex gap-4">
                      <div
                        className={`p-3 shrink-0 rounded-2xl h-fit ${
                          tx.type === "OUT"
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-black text-gray-900 dark:text-white leading-snug">
                          <span className="text-blue-500 break-all shrink">
                            @
                            {tx.users?.full_name ||
                              tx.users?.email.split("@")[0]}
                          </span>{" "}
                          님이
                          <span className="mx-1 font-extrabold text-indigo-600 dark:text-indigo-400">
                            {tx.items?.name}
                          </span>{" "}
                          {tx.quantity}개를
                          {tx.type === "OUT"
        ...
        >
          ...
          <div className="space-y-4">
            {warehouses?.map((wh: any, idx: number) => {
              const whItems = inventory?.filter(
                (inv: any) => inv.warehouse_id === wh.id,
              );
              const count =
                whItems?.reduce(
                ...
                >
                  ...
                  <div className="w-full bg-gray-100 dark:bg-zinc-800 h-2 rounded-full overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{
                        width: `${Math.min((count / totalItems) * 100, 100)}%`,
                      }}
