...
export async function POST(
  ...
  const parsed = moveProductSchema.safeParse(body)
  if (!parsed.success) {
  ...
  const { newLocation, notes } = parsed.data
  ...
  if (product.status !== "ACTIVE") {
    return NextResponse.json(
      { error: "BUSINESS_RULE_VIOLATION", message: "ACTIVE 상태의 물품만 위치를 이동할 수 있습니다." },
      { status: 422 },
    )
  }
  ...
  const updatedProduct = await prisma.$transaction(async (tx) => {
    await tx.productHistory.create({
      data: {
        productId,
        actionType: "MOVE",
        previousLocation: product.storageLocation,
        newLocation,
        performedBy: userId,
        notes: notes ?? null,
      },
