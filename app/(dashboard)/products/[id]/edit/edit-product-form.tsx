...
function createInitialState(product: ProductEditFormData): ProductFormState {
  return {
    ...
    usefulLife: product.usefulLife === null ? "" : String(product.usefulLife),
    storageLocation: normalizeStorageLocation(product.storageLocation),
    photoUrl: product.photoUrl ?? "",
    notes: product.notes ?? "",
  }
}

export function EditProductForm({ product }: { product: ProductEditFormData }) {
    ...
    try {
      const response = await fetch(`/api/products/${product.id}`, {
        ...
        body: JSON.stringify({
          ...
          ...(photoTouched
            ? { photoUrl: form.photoUrl.trim() ? form.photoUrl : null }
            : {}),
        }),
      })
      ...
      setSuccess("물품 정보가 수정되었습니다. 상세 화면으로 이동합니다.")
      ...
      setError("네트워크 오류로 물품을 수정하지 못했습니다.")
    } finally {
      setLoading(false)
    }
  }
    ...
    <section className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h2 className="mt-3 text-2xl font-semibold tracking-tight text-slate-950">물품 수정</h2>
          <p className="mt-2 text-sm leading-6 text-slate-600">
            기존 물품의 기본 정보와 보관 정보를 수정합니다.
          </p>
        </div>
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 p-6 sm:p-8">
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <div className="space-y-3 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4">
                <input
                  ...
                  className="block w-full text-sm text-slate-700 file:mr-4 file:rounded-2xl file:border-0 file:bg-slate-950 file:px-4 file:py-2 file:text-sm file:font-medium file:text-white hover:file:bg-slate-800"
                />
                <p className="text-xs leading-5 text-slate-500">
                  이미지는 자동으로 리사이즈/압축되어 저장됩니다.
                </p>
                {form.photoUrl ? (
          ...
          <div className="flex flex-col gap-3 border-t border-slate-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
            ...
            >
              {photoProcessing ? "사진 처리 중..." : loading ? "수정 중..." : "수정하기"}
            </button>
          </div>
        </form>
      </div>
